...
def render_structured_report(report_md, df_view, kpis, cleaning_report, insights):
    """Parse markdown report text and render a structured HTML report card.

    Falls back to generated sections if the LLM report is unstructured.
    """
    import re

    # Extract headline sections starting with '##'
    sections = {}
    if report_md:
        parts = re.split(r"^##+\s*", report_md, flags=re.MULTILINE)
        # parts[0] is leading text before first header
        if parts:
            if parts[0].strip():
                sections['Overview'] = parts[0].strip()
            for p in parts[1:]:
                # split section into title + body
                lines = p.splitlines()
                title = lines[0].strip()
                body = "\n".join(lines[1:]).strip()
                if title:
    ...
    # If LLM didn't provide useful sections, synthesize basics
    if not sections:
        sections = {
            'Overview': insights or 'Analysis complete. See recommendations below.',
            'Data Cleanup': '\n'.join(cleaning_report) if cleaning_report else 'No cleaning required.',
            'Business Read': insights or 'No concise insights available.',
            'Recommended Next Steps': '- Review data quality\n- Validate top anomalies\n- Track key metrics weekly',
            'Closing Note': 'Report generated by AnalystAI.'
        }
    ...
    toc = ''
    if sections:
        toc += '<div style="width:100%;margin-bottom:12px;">'
        toc += '<strong>Contents</strong>: '
        toc += '<span style="color:#64748B;margin-left:8px;">'
        for title in sections.keys():
            slug = _slug(title)
            toc += f"<a href='#report-{slug}' style='margin-right:8px;color:#4F46E5;text-decoration:none;'>{title}</a>"
        toc += '</span></div>'
        html = toc + html
    # Left column: Overview + KPIs
    left = '<div style="flex:1;min-width:360px">'
    slug_o = _slug('Overview')
    left += f'<div id="report-{slug_o}" style="scroll-margin-top:120px;margin-bottom:12px;"><h3 style="margin:0 0 6px 0;">📋 Overview</h3>'
    left += f"<div style='color:#0F172A;font-size:0.95rem;line-height:1.5'>{md_to_html(sections.get('Overview',''))}</div></div>"

    # KPIs
    left += '<div style="margin-bottom:12px;"><h4 style="margin:0 0 8px 0;">Key metrics</h4>'
    left += '<div class="kpi-grid" style="margin-top:6px;">'
    for k in (kpis or []):
        left += f"<div class='kpi-card'><div class='kpi-lbl'>{k['label']}<span class='kpi-ico'>{k.get('icon','')}</span></div><div class='kpi-val'>{k['value']}</div><div class='kpi-sub'>{k.get('sub','')}</div></div>"
    left += '</div></div>'
    left += '</div>'

    # Right column: Data Cleanup, Business Read, Recommendations
    right = '<div style="flex:1.5;min-width:420px">'
    # Data Cleanup
    slug_dc = _slug('Data Cleanup')
    right += f'<div id="report-{slug_dc}" style="scroll-margin-top:120px;margin-bottom:12px;"><h4 style="margin:0 0 6px 0;">🧹 Data Cleanup</h4>'
    cleanup = sections.get('Data Cleanup') or '\n'.join(cleaning_report or [])
    if cleanup:
        # render bullets
        bullets = cleanup.strip().splitlines()
        right += '<ul style="color:#334155">'
        for b in bullets:
            b = b.strip().lstrip('-•')
            if b:
                right += f"<li style='margin-bottom:6px;'>{b}</li>"
        right += '</ul>'
    else:
        right += '<div style="color:#64748B">No cleaning steps recorded.</div>'
    right += '</div>'

    # Business Read
    br = sections.get('Business Read') or sections.get('Business Read:', '')
    if br:
        slug_br = _slug('Business Read')
        right += f'<div id="report-{slug_br}" style="scroll-margin-top:120px;margin-bottom:12px;"><h4 style="margin:0 0 6px 0;">📈 Business Read</h4>'
        right += f"<div style='color:#334155'>{md_to_html(br)}</div></div>"

    # Recommendations
    rec = sections.get('Recommended Next Steps') or sections.get('Recommendations') or ''
    if rec:
        slug_rec = _slug('Recommended Next Steps')
        right += f'<div id="report-{slug_rec}" style="scroll-margin-top:120px;margin-bottom:12px;"><h4 style="margin:0 0 6px 0;">🚀 Recommended Next Steps</h4>'
        # If the rec block contains lines, render as numbered list
        items = [r.strip().lstrip('-•0123456789. ') for r in rec.splitlines() if r.strip()]
        if items:
            right += '<ol style="color:#334155">'
            for it in items:
                right += f"<li style='margin-bottom:6px;'>{it}</li>"
            right += '</ol>'
        else:
            right += f"<div style='color:#64748B'>{md_to_html(rec)}</div>"
        right += '</div>'

    # Closing note
    cn = sections.get('Closing Note') or ''
    if cn:
        right += '<div style="margin-top:8px;padding:10px;background:#F8FAFC;border-radius:8px;border:1px solid #E8F0FF;">'
        right += f"<strong>✅ Closing note:</strong> {md_to_html(cn)}</div>"

    right += '</div>'

    html += left + right + '</div>'
    # Wrap in report-card
    # Appendix with raw markdown
    append = ''
    try:
        append += '<div style="margin-top:12px;">'
        append += '<details><summary style="cursor:pointer;font-weight:600;color:#475569">Appendix — Raw report and data</summary>'
        append += '<div style="margin-top:8px;color:#334155;">'
        if report_md:
            append += f"<h5 style='margin:8px 0 6px 0;'>Full report (raw)</h5><div style='font-size:0.95rem'>{md_to_html(report_md)}</div>"
        if df_view is not None:
            # small data snapshot
            try:
                sample = df_view.head(6).to_html(classes='table', index=False)
                append += '<h5 style="margin:12px 0 6px 0;">Data sample</h5>' + sample
            except Exception:
                pass
    ...
    # Add small script to highlight active section in the TOC
    script = """
    <script>
    (function(){
      try{
        const tocLinks = Array.from(document.querySelectorAll('.report-card .report-toc a'));
    ...
    return f"<div class='report-card'>{html}{append}{script}</div>"
...
if df is not None:
    ...
    <div class="app-hero">
        ...
        <div class="hero-actions">
            ...
            <span class="hero-pill">Powered by OpenAI Codex</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # Render a structured report card (parse LLM markdown into sections)
    try:
        report_html = render_structured_report(
            st.session_state.report,
            df_view,
            kpis,
            st.session_state.cleaning_report or [],
        ...
        st.markdown(f'<div class="report-card">{md_to_html(st.session_state.report)}</div>', unsafe_allow_html=True)
